...
//---------------------------------------------------------------------
{
    ...
    else if ( cmd == "HAM" )
    {
        ...
        for ( unsigned int i=0; i<vHAM.size( ); i+=unsigned int(vArgs[0]) )
        {
            for ( unsigned int k=0; k<unsigned int(vArgs[0]); ++k )
            {
                if ( k == int(vArgs[0])-1 )
                {
            ...
            fprintf( stdout, "\n" );
            //pork.push_back( Vector_3( vHAM[i], vHAM[i+1], vHAM[i+2] ) );
        }

        if ( bPrint )
        {
            //WriteVector_3File( pork );
        }
        else
        {
            //SVG_Output( 500, pork );
