...
std::string Cividis::OrdinalToCividisHexString(const unsigned long n) {
   ...
   ostr << std::setw(2) << std::setfill('0') << r << g << b;

   return "#" + ostr.str();
}

std::string Cividis::CividisScaleSVG( const int height, const int strokeWidth) {
   std::ostringstream ostr;

   for ( unsigned long i=0; i<255; ++i ) {
      const int yvalue = height - 2*i;
      ostr <<
         "<line fill=\"none\" stroke=\"" << OrdinalToCividisHexString(i) << "\" stroke-width=\"3\" " <<
         "x1=\"" << 0 << "\" x2=\"" << 80 << "\" y1=\"" << yvalue << "\" y2=\""
          << yvalue << "\" />"
         << std::endl;
   }
      return ostr.str();
